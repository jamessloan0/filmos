...
export default function AdminTesters() {
  ...
  const { data: pendingTesters = [], isLoading: loadingPending } = useQuery({
    ...
    queryFn: () => base44.entities.PendingTester.list(),
    enabled: !!user,
  });
  ...
  const handleAddPending = async (e) => {
    ...
    await base44.entities.PendingTester.create({ email: newEmail.trim().toLowerCase() });
    setNewEmail("");
  ...
  const handleRemovePending = async (id) => {
    await base44.entities.PendingTester.delete(id);
    queryClient.invalidateQueries({ queryKey: ["pending-testers"] });
  };
  ...
  const handlePromoteAll = async () => {
    setPromoting(true);
    try {
      const res = await base44.functions.invoke("promotePendingTesters", { manualTrigger: true });
