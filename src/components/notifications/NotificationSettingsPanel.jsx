...
export default function NotificationSettingsPanel({ projectId, user }) {
  ...
  const { data: settings = [] } = useQuery({
    ...
    queryFn: () => base44.entities.NotificationSettings.filter({ project_id: projectId, user_email: user?.email }),
  ...
  const handleSave = async () => {
    ...
    if (existing) {
      await base44.entities.NotificationSettings.update(existing.id, local);
    } else {
