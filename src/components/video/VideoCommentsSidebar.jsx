...
export default function VideoCommentsSidebar({ comments, onSeek, onDelete, currentUserName }) {
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="flex items-center gap-2 px-4 py-3 border-b border-zinc-100">
        ...
        <span className="text-sm font-semibold text-zinc-700">Timeline Comments</span>
        {comments.length > 0 && (
      ...
      {sorted.length === 0 ? (
        ...
        <div className="flex-1 overflow-y-auto divide-y divide-zinc-50">
          {sorted.map((c) => (
            <div
              key={c.id}
              className="group px-4 py-3 hover:bg-zinc-50 transition-colors cursor-pointer"
              ...
              <div className="flex items-start justify-between gap-2">
                <div className="flex items-start gap-2.5 min-w-0">
                  ...
                  <div className="min-w-0">
                    <p className="text-sm text-zinc-800 leading-snug">{c.text}</p>
                ...
                {onDelete && (c.author === currentUserName) && (
                  <button
                    ...
                    className="opacity-0 group-hover:opacity-100 p-1 rounded text-zinc-300 hover:text-red-400 transition-all flex-shrink-0"
                  >
